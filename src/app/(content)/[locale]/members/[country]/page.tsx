...
import { getAllMembers, Member } from "@/src/lib/members";
...
function getCountryName(countryCode: string): string {
  try {
    const countriesPath = join(process.cwd(), 'public', 'countries.json');
    const countriesData = JSON.parse(readFileSync(countriesPath, 'utf-8'));
    if (countriesData?.objects?.world?.geometries) {
      const country = countriesData.objects.world.geometries.find(
        (geo: any) => geo.id === countryCode
      );
      return country?.properties?.name || countryCode;
    }
  } catch (error) {
    // Fallback to country code if parsing fails
  }
  return countryCode;
}
      ...
      <Section
        headline={
          <div>
            <Link
              href="/members"
              className="mb-4 inline-block text-gray-900 underline underline-offset-4 font-semibold"
            >
              ← Back to all members
            </Link>
            <Heading>Members from {countryName}</Heading>
          </div>
        }
        subheadline={
          <Text>
            {countryMembers.length} {countryMembers.length === 1 ? 'member' : 'members'} found
          </Text>
        }
      >
